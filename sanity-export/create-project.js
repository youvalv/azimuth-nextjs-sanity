...
async function createProject({ projectName, dataset, token }) {
    if (!token) {
        throw new Error('no sanity token provided, either provide an API token via "SANITY_TOKEN" environment variable or login into sanity using sanity cli "sanity login"');
    }
    if (!dataset) {
